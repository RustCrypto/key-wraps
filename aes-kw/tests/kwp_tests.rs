use aes_kw::{Error, KekAes128, KekAes256, KeyInit};
use aes_kw::{KekAes192, IV_LEN};
use hex_literal::hex;
use std::assert_eq;
...
fn padding_cleared() {
    ...
    let kek = KekAes192::new(&key.into());
...
fn error_invalid_data_size() {
    ...
    let kek = KekAes128::new(&key.into());
...
fn error_invalid_kek_size() {
    ...
    let result = KekAes128::new_from_slice(&key);
    assert!(result.is_err());
}
